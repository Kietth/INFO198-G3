...
std::vector<std::string> headFiles = {"green", "blue", "pink", "skyblue", "red", "yellow"};
std::vector<sf::Color> playerColors = {sf::Color::Green, sf::Color::Blue, sf::Color(255, 105, 180), sf::Color::Cyan, sf::Color::Red, sf::Color::Yellow };
std::map<int, sf::Texture> headTextures;
std::map<int, sf::Sprite> headSprites;
std::map<int, sf::Texture> bodyTextures;
std::map<int, sf::Sprite> bodySprites;
...
bool cargarAssets() {
    if (!font.loadFromFile("/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf")) {
        if (!font.loadFromFile("/usr/share/fonts/truetype/liberation/LiberationSans-Regular.ttf")) {
            std::cerr << "Error: Fuente no encontrada." << std::endl;
            return false;
        }
    }
    if (!titleTexture.loadFromFile("assets/title.png")) { std::cerr << "Error assets/title.png\n"; return false; }
    titleSprite.setTexture(titleTexture);
    titleSprite.setOrigin(titleTexture.getSize().x/2.f, titleTexture.getSize().y/2.f);
    titleSprite.setScale(3.f, 3.f);

    if (!foodTexture.loadFromFile("assets/food.png")) { std::cerr << "Error assets/food.png\n"; return false; }
    foodSprite.setTexture(foodTexture);

    for(size_t i = 0; i < headFiles.size(); ++i) {
        if (!headTextures[i].loadFromFile("assets/heads/" + headFiles[i] + ".png")) { std::cerr << "Error assets/heads/\n"; return false; }
        headSprites[i].setTexture(headTextures[i]);
        headSprites[i].setOrigin(headTextures[i].getSize().x/2.f, headTextures[i].getSize().y/2.f);

        if (!bodyTextures[i].loadFromFile("assets/body/" + headFiles[i] + ".png")) { std::cerr << "Error assets/body/\n"; return false; }
...
void drawGrid(sf::RenderWindow& window) {
    ...
    float offset_x_px = -(std::fmod(camera_grid_x, 1.0f)) * TILE_SIZE;
    float offset_y_px = -(std::fmod(camera_grid_y, 1.0f)) * TILE_SIZE;
...
void drawText(sf::RenderWindow& window, const std::string& str, float x, float y, int size, sf::Color color) {
...
void drawTextCentered(sf::RenderWindow& window, const std::string& str, int y_offset, int size, sf::Color color) {
...
// --- FUNCIÓN MODIFICADA (TEXTO DE LOBBY) ---
void dibujarEstado(sf::RenderWindow& window) {
    ClientState state_copy_draw = g_client_state.load();
    std::string lobby_msg_copy_draw;
    // bool am_i_alive = false; // <-- Eliminado
    {
        std::lock_guard<std::mutex> lock(state_mutex);
        lobby_msg_copy_draw = g_lobby_message;
        // int my_id = g_my_player_id.load(); // No necesario aquí
        // for(const auto& snake : current_state.all_snakes) {
